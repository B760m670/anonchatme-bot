...
export default class SamuraiWorldScene extends Phaser.Scene {
  ...
  private spawnNpc(def: NpcDef, groundY: number): void {
    ...
    const x = def.x, gy = groundY;
    const c = def.color;

    // Shadow
    g.fillStyle(0x000000, 0.2);
    g.fillEllipse(x, gy, 24, 6);

    // Legs
    g.fillStyle(0x1a1a2a, 0.9);
    g.fillRect(x - 7, gy - 22, 6, 20);
    g.fillRect(x + 1, gy - 22, 6, 20);

    // Robe (wide, flowing)
    g.fillStyle(c, 0.75);
    g.fillRect(x - 10, gy - 40, 20, 22);

    // Sash
    const dkC = (()=>{
      const r=((c>>16)&0xff),gv=((c>>8)&0xff),b=c&0xff,f=0.6;
      return (Math.floor(r*f)<<16)|(Math.floor(gv*f)<<8)|Math.floor(b*f);
    })();
    g.fillStyle(dkC, 0.9);
    g.fillRect(x - 10, gy - 28, 20, 5);

    // Arms
    g.fillStyle(c, 0.8);
    g.fillRect(x - 16, gy - 38, 7, 16);
    g.fillRect(x + 9,  gy - 38, 7, 16);

    // Neck
    g.fillStyle(0xdaa87a, 1);
    g.fillRect(x - 3, gy - 44, 6, 6);

    // Head
    g.fillStyle(0xdaa87a, 1);
    g.fillCircle(x, gy - 52, 10);

    // Hair (dark, simple)
    g.fillStyle(0x1a0a00, 1);
    g.fillCircle(x, gy - 52, 10);
    g.fillRect(x - 9, gy - 60, 18, 8);
    g.fillStyle(0xdaa87a, 1);
    g.fillCircle(x + 1, gy - 51, 9);

    // Simple eyes
    g.fillStyle(0x1a0a00, 0.8);
    g.fillCircle(x - 3, gy - 51, 2);
    g.fillCircle(x + 4, gy - 51, 2);
